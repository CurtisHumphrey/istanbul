...
function compareTimestamps (newestTimestamp, node) {
  var nodeTime = node.getTimestamp()
  return (nodeTime > newestTimestamp) ? nodeTime : newestTimestamp
}
...
Node.prototype = {
    ...
    getTimestamp: function () {
      ...
      return this.children.reduce(compareTimestamps, (new Date(0)).getTime())
    },
    fullPath: function () {
...
TreeSummary.prototype = {
    ...
    convertToTree: function (summaryMap, arrayPrefix) {
        ...
        Object.keys(summaryMap).forEach(function (key) {
            ...
            node.timestamp = (new Date(fs.statSync(key).ctime)).getTime()
            seen[key] = node;
            nodes.push(node);
